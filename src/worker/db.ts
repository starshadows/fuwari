...
const MIGRATIONS: Migration[] = [
	...
	{
		version: "0011",
		description: "Allow succeeded content deploy status",
		statements: [
   ...
   END`,
		...
		isApplied: (env) => contentDeployStatusAllowsValue(env, "succeeded"),
	},
];
...
async function contentDeployStatusAllowsValue(
	env: Env,
	value: string,
): Promise<boolean> {
	try {
		...
		if (!sql) return false;
		const match =
			/CHECK\s*\(\s*"?deploy_status"?\s+IN\s*\(([^)]*)\)\s*\)/i.exec(sql);
		if (!match) return true;
		return match[1]
			.split(",")
			.map((entry) => entry.trim().replace(/^['"]|['"]$/g, ""))
