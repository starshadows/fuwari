...
async function getStatsSummary(
  ...
  const cutoff = new Date(
    Date.now() - STATS_ACTIVE_WINDOW_MS,
  ).toISOString();
  await cleanupInactiveVisitors(env, cutoff);

  const [siteTotals, siteToday, siteYesterday, siteMonth, siteVisitors, activeVisitors, pageTotals, pageToday, pageVisitors] =
    await Promise.all([
      env.DB.prepare("SELECT COALESCE(SUM(pv), 0) AS totalPv FROM stats_site_daily").first<{ totalPv: number }>(),
      env.DB.prepare("SELECT COALESCE(pv, 0) AS pv, COALESCE(uv, 0) AS uv FROM stats_site_daily WHERE day = ?").bind(day).first<{ pv: number; uv: number }>(),
      env.DB.prepare("SELECT COALESCE(pv, 0) AS pv FROM stats_site_daily WHERE day = ?").bind(yesterday).first<{ pv: number }>(),
      env.DB.prepare("SELECT COALESCE(SUM(pv), 0) AS pv FROM stats_site_daily WHERE day >= ? AND day <= ?").bind(monthStart, day).first<{ pv: number }>(),
      env.DB.prepare("SELECT COUNT(*) AS totalUv FROM stats_visitors").first<{ totalUv: number }>(),
      env.DB.prepare("SELECT COUNT(*) AS count FROM stats_active_visitors WHERE last_seen >= ?").bind(cutoff).first<{ count: number }>(),
      env.DB.prepare("SELECT COALESCE(SUM(pv), 0) AS totalPv FROM stats_page_daily WHERE path = ?").bind(path).first<{ totalPv: number }>(),
      env.DB.prepare("SELECT COALESCE(pv, 0) AS pv, COALESCE(uv, 0) AS uv FROM stats_page_daily WHERE path = ? AND day = ?").bind(path, day).first<{ pv: number; uv: number }>(),
      env.DB.prepare("SELECT COUNT(*) AS totalUv FROM stats_page_visitors WHERE path = ?").bind(path).first<{ totalUv: number }>(),
    ]);
...
async function cleanupInactiveVisitors(
  ...
  await env.DB.prepare(
    "DELETE FROM stats_active_visitors WHERE last_seen < ?",
  )
    .bind(cutoff)
    .run();
}
