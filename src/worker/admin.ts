...
import { FRIEND_STATUSES, ALLOWED_AVATAR_MIME_TYPES, MAX_AVATAR_SIZE, TELEGRAM_SETTINGS_KEY } from "./constants";
import { getCommentsConfig } from "./comments";
import { writeTelegramSettings, sendTelegramMessage } from "./friends";
import { listAdminMusic, listR2MusicObjects, importR2MusicObjects, createMusicTrack, updateMusicTrack, deleteMusicTrack } from "./music";
...
export async function handleAdminApi(
  ...
  if (segments[2] === "settings") {
    ...
    if (segments[3] === "telegram") {
      ...
      if (segments[4] === "test" && request.method === "POST") return sendAdminTelegramTest(env);
    }
  }
  ...
  // Music
  if (segments[2] === "music") {
    if (segments[3] === "objects" && request.method === "GET") return listR2MusicObjects(env);
    if (segments[3] === "import" && request.method === "POST") return importR2MusicObjects(request, env);
...
async function listAdminFriends(
  env: Env,
  requestUrl: URL,
): Promise<Response> {
...
async function updateFriend(
  ...
  if (typeof body.name === "string") { fields.push("name = ?"); values.push(readString(body.name, 40)); }
  if (typeof body.description === "string") { fields.push("description = ?"); values.push(readString(body.description, 120)); }
  if (typeof body.url === "string") {
    const v = readString(body.url, 400);
    if (!isHttpsUrl(v)) return json({ error: "链接必须是 https 地址。" }, 400);
    fields.push("url = ?");
    values.push(v);
  }
  if (typeof body.avatarUrl === "string") {
    const v = readString(body.avatarUrl, 600);
    if (!isAvatarUrl(v)) return json({ error: "头像地址不正确。" }, 400);
...
async function uploadAvatar(request: Request, env: Env): Promise<Response> {
  ...
  if (!ALLOWED_AVATAR_MIME_TYPES.has(file.type)) {
    return json(
      { error: "头像必须是 JPG、PNG、WebP、AVIF 或 GIF 图片。" },
      400,
    );
  }
